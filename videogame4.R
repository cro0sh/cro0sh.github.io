...
trainDate <- video %>% filter(Date <= 2014) %>% select(-Year_of_Release)
testDate <- video %>% filter(Date == '2015' | Date == '2016') %>% select(-Year_of_Release)
...
linearReg2 <- train(logNA_Sales ~ log(Critic_Score) + Platform + Genre + Publisher + log(Critic_Count) + log(User_Score) + log(User_Count) + Rating + Date, data=trainDate2, method = "lm", trControl = trainControl(method="cv"), allowParallel = T)
...
rfMod2 <- train(logNA_Sales ~ log(Critic_Score) + Platform + Genre + Publisher + log(Critic_Count) + log(User_Score) + log(User_Count) + Rating + Date, data=trainDate2, method = "rf", trControl = trainControl(method="cv"), num.trees = 1000, allowParallel = T)


CARTmod3 <- train(NA_Sales ~ ., data=trainDate, method = "rpart", trControl = trainControl(method="cv"))
CARTmod4 <- train(logNA_Sales ~ log(Critic_Score) + Platform + Genre + Publisher + log(Critic_Count) + log(User_Score) + log(User_Count) + Rating + Date, data=trainDate2, method = "rpart", trControl = trainControl(method="cv"))
...
resamp2 <- resamples(list("Linear Regression" = linearReg, "Tree" = CARTmod3))
summary(resamp2)

# resamp2 <- resamples(list("Linear Regression" = linearReg2, "Tree" = CARTmod4, "gbm" = gbm4, "rf2" = rfMod2))
# summary(resamp2)
...
plotRMSE <- ggplot(data = dftest, mapping = aes(x = type, y = rmse)) + geom_bar(aes(fill=type), color="black", stat="identity") + ggtitle('RMSE Over Different Models') + xlab('model') + coord_flip() #+ geom_point(size=5,alpha=.7) + coord_flip() + xlab('model') 
plotRMSE

plotrq <- ggplot(data = dftest, mapping = aes(x = type, y = rsquared)) + geom_bar(aes(fill=type), color="black", stat="identity") + ggtitle('RSquared Over Different Models') + xlab('model') + coord_flip() #geom_point(size=5,alpha=.7) + xlab('model') + ggtitle('RSquared Over Different Models') + coord_flip()
plotrq

multiplot(plotRMSE, plotrq)
...
plot65 <- ggplot(df1) + geom_point(mapping = aes(x = Sales, y = rfpred, colour = rfpred), size=5,alpha=.7) + geom_abline(intercept=0, slope=1) + xlab('observed') + ylab('predicted') + scale_color_continuous(name="Predicted") + ggtitle('Predicted vs Observed Random forest') + scale_color_viridis()
plot65
...
plotlmpo <- ggplot(df1) + geom_point(mapping = aes(x = Sales, y = lmpred, color=lmpred), size=5,alpha=.7) + geom_abline(intercept=0, slope=1) + xlab('observed') + ylab('predicted') + ggtitle('Predicted vs Observed Linear Model') + scale_color_viridis()
plotlmpo

# plotlmpo <- ggplot(df1) + geom_point(mapping = aes(x = Sales, y = lmpred, fill = lmpred), size=5,alpha=.25) + geom_abline(intercept=0, slope=1) + xlab('observed') + ylab('predicted') + scale_color_continuous(name="Predicted") + ggtitle('Predicted vs Observed Linear Model') + scale_fill_viridis()
# plotlmpo

plot66 <- ggplot(dfresiduals) + geom_point(mapping = aes(x = observed, y = residuals, colour = residuals), size=5,alpha=.7) + xlab('observed') + ylab('residuals') + geom_hline(yintercept=0) + ggtitle('Residuals vs Observed of Linear Model') + scale_color_viridis()
plot66

multiplot(plotlmpo, plot66, plot65)
